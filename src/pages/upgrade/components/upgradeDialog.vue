<template>
  <el-dialog :title="`强化模拟 - ${row.title}`" :visible="visible" @close="handleCancel" width="80%">
    <view class="dialog-container">
      <!-- 强化类型 -->
      <view class="upgrade-type-button">
        <el-switch v-model="isStrong" active-text="强化特惠" inactive-text="普通强化"></el-switch>
      </view>
      <!-- 强化信息展示 -->
      <el-image :src="$fn.toWebp(row.cover)" :preview-src-list="[$fn.toWebp(row.cover)]"
        class="dialog-cover"></el-image>
      <view v-if="level < 10">
        <text class="level-info">等级{{ level }} - 等级{{ level + 1 }}</text>
        <text>强化成功几率：{{ success + '%' }}</text>
      </view>
      <view v-else>已满级</view>
      <!-- 强化进度 -->
      <view class="upgrade-progress">
        <el-progress :text-inside="true" :stroke-width="30" :percentage="percent" class="progress" :format="formatText"
          status="success"></el-progress>
        <view>满祝福值：{{ row.info && row.info[level] }}</view>
      </view>
      <!-- 操作按钮 -->
      <view class="upgrade-button">
        <el-button @click="upgrade" :disabled="level === 10">强化</el-button>
      </view>
      <!-- 统计 -->
      <view>您已花费{{ money }}个黄金卷轴</view>
    </view>
